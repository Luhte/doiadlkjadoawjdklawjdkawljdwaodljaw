...
async function registerCommands(client: Client<true>) {
    ...
    for (const guildId of client.guilds.cache.keys()) {
        await registerCommandsGuild(client, guildId);
    }

    log.info('slash commands updated');
}

async function registerCommandsGuild(client: Client<true>, guildId: Snowflake) {
...
Events.on('botReady', async (client) => {
    ...
    client.on('guildCreate', async (guild) => {
        await registerCommandsGuild(client, guild.id);
    });
